...
if [ -z "$1" ]; then
    echo "Usage: $0 [ACCOUNT SUBFOLDER] (RESOURCE)"
    exit 1
else
  PROJECT=$1
fi
...
# Only use provided resource, otherwise use all resources
if [ -n "$RESOURCE" ]; then
    RESOURCES=(
      "${RESOURCE}"
    )
...
for i in "${!RESOURCES[@]}"; do
  ...
  cf-terraforming import --zone ${ZONE_ID} --resource-type "${RESOURCES[$i]}" --terraform-binary-path=./terraform --modern-import-block >> import.raw
  echo -e "\nProcessing ${RESOURCES[$i]} for Account: ${ACCOUNT_ID}"
  cf-terraforming import --account ${ACCOUNT_ID} --resource-type "${RESOURCES[$i]}" --terraform-binary-path=./terraform --modern-import-block >> import.raw
done
